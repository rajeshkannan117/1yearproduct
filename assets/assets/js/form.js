(function(w,d,$){
	$(d).ready(function(){
		...
		var comptField = 1;
		var dialog;
		var todrag = $("#basic li span,#group li div,#advanced li span");
		var pages = $("#pages");
		var cols = $(".connected"); 
		todrag.draggable({
	      ...
	      revert:'invalid',
      	  start:function(){
      	  	...
      	  	$('#content_right').find('.formfields').remove();	
...
///max height
		      	...
		      	"ok" :function(){
		      		...
		      		switch(type){
		      			...
		      			case 'date':
		                        ...
		                        var select_format = $('#date_formats_'+date_id[1]).val();
		                        ...
		                        $('#cur_date_'+date_id[1]).val(date_string);
		      					$('#sel_date_format',current).val(select_format);
		      				break;	
		      			case 'time':
		      				var select = '';
		      				var time_id = popupid.split('_');
                        	var sel_time_format = $('#time_formats_'+time_id[1]).val();
                        	var select_time_format = $('#time_formats_'+time_id[1]).val();
	                        if(sel_time_format =='hh:mm:ss'){
	                            ...
	                            $('#time_value_'+time_id[1]).val(time);
		      					...
		      					$('#sel_time_format',current).val(select_time_format);
		      				break;
		...
		var common = {
        	...
        	build:function(){
                ...
                //if($rows.find('ul.connected li').length)
                    ...
                    if(length != 0){
                        ...
                        $(row).each(function(c,columns){
                            ...
                            */
                            ...
                            if($(columns).find('#sel_time_format').length > 0){
                                fielddata.format = $('#sel_time_format').val();
                            }
                            if($(columns).find('#sel_date_format').length > 0){
                                fielddata.format = $('#sel_date_format').val();
                            }
        	...
        	hasScrollBar :function(current) {
        		return current.get(0).scrollHeight > current.height();
    		}
        }
	});

...
class Role_model extends CI_Model {
	...
	public function role_info($role_id){
		...
		$this->db->select('r.role_id,r.role_name,r.role_desc,r.default,r.organiser_id,r.status,r.created_by')
				->from('roles r')
	...
	public function role_list(){
                $org_id = $this->session->userdata('org_id');
		$this->db->select('r.role_id,r.role_name,r.role_desc,r.default,r.status,r.created_by')
			->from('roles r')
                        ->where('r.role_id != 1')
                        //->join('roles_permissions rp','rp.role_id = r.role_id')
                       // ->join('roles_category_type rc','rc.roles_category_type = rp.roles_category_type')
                        ->where('r.organiser_id',$org_id);
                        //->where('r.status','1');
		$res = $this->db->get();

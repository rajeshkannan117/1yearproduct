 ...
 */
class Template {
    public function component_templates($col){
        $res = '';
        if(is_object($col)){
            $type =  $col->type;
            switch($type){
                ...
                case 'time':
                case 'file':
                    $res .= $this->templates($col);
                    break;
                case 'radio':
                case 'select':
                case 'checkbox':
                    $res .= $this->template_multi($col);
                    break;
            }
        return $res;
        }
    ...
    public function radius($v){
        $radius ='';
        $radius .='<div class="uk-form-row">
                        <span class="uk-form-labels">Radius Limit</span>
                        <div class="uk-form-controls uk-form-controls-text">
                            <input type="number" name="radius" id="radius_'.$v->name.'" value="'.$v->radius.'" />
                        </div>
                   </div>';
        return $radius;
}
    ...
    public function templates($v){
        $title = preg_replace('/\s+/', '', $v->title).'_'.$v->formfieldid;
        $templates= '<div class="uk-width-1-1 cols '.$v->type.'_'.strtolower($title).'">';
        $templates.= '<div class="uk-panel config_'.$v->type.' portlet">';
        $templates.=  '<div class="portlet-header" data-type="'.$v->type.'" data-fieldid ="'.$v->fieldid.'">
                        <div class="uk-grid">
                            <div class="uk-width-1-2">
                                <label class="title_label">'.$v->title.'</label><br/>
                                 <input type="'.$v->type.'"/>
                                <div class="uk-width-1-2 action" id="">
                                    <i class="uk-icon-remove delete_field"></i>
                                    <input type="hidden" class="formfieldid" value="'.$v->formfieldid.'">
                                    <input type="checkbox" id="'.$v->type.'_check_'.$v->formfieldid.'" class="required" data-type="'.$v->type.'">Required
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    public function template_multi($v){
            $title = preg_replace('/\s+/', '', $v->title).'_'.$v->formfieldid;
            //$tileedits_
            $templates= '<div class="uk-width-1-1 cols '.$v->type.'_'.strtolower($title).'">';
            $templates.= '<div class="uk-panel config_'.$v->type.' portlet">
             <div class="portlet-header" data-type="'.$v->type.'" data-fieldid ="'.$v->fieldid.'">
                    <label class="title_label">'.$v->title.'</label>
                    <div class="uk-grid">
                        <div class="uk-width-1-2">'.
                            $this->choices($v)
                        ...
                        <div class="uk-width-1-2 action" style="display: none;" id="".>
                            <i class="uk-icon-remove delete_field"></i>                     
    ...
    public function popup_options($v){
        $options = '
        <div class="options" id="choices_'.$v->formfieldid.'" style="display:none">
            <div class="input_fields_wrap">
                <a class="add_field_button md-btn md-btn-primary">
                    Add More
                </a>
            <element>';
            foreach($v->choices as $k=>$optionss) {
        $options .='<p>
    <input type="text" name="select[]" class="select_label" data-optionid="'.$optionss->id.'" value ="'.$optionss->title.'"/>
        <a href="#" class="remove_field ui-button-icon ui-icon ui-icon-closethick"></a>
                </p>';
            }
        $options .='</element>
            </div>
    ...
    public function choices($v){
        $choices ='
                <div class="title_'.$v->type.' select">';
                    if($v->type == 'select'){
                        $choices .='<select class="choices_'.$v->formfieldid.'">';
                        foreach($v->choices as $k=>$options){
                            $choices .='<option class="title_value" data-optionid="'.$options->id.'">'.
                                    $options->title
                            .'</option>';
                        }    
                        $choices .='</select>';
                        ...
                        foreach($v->choices as $k=>$options){
                            $choices .='<input type="'.$v->type.'" /> 
                            <span class="title_value" data-optionid="'.$options->id.'">'.
                                    $options->title
                            .'</span>';
                        }
                    }
...
/* Plae picker Components */
public function input_placepicker($v){
    $place = '<div class="'.$v->type.' cols">
            <div class="uk_panel config_'.$v->type.' portlet">
            <div class="portlet-header">
                <div class="uk-grid">
                    <div class="uk-width-1-2">
                <label class="title_label">'.$v->title.'</label>
            ...
            <div class="portlet-content portlet-content-'.$v->name.' uk-hidden" type="'.$v->type.'">
                <div class="tab">
                    <ul class="uk-tab" data-uk-tab="{connect:\'#my-id-'.$v->name.'\'}">
                        <li><a href="#tabs-1">Informations</a></li>
                        <li><a href="#tabs-3">Validation rules</a></li>
